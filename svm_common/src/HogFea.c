   ...
   // FILE *file = fopen("./Hig.txt", "w");
    ...
    for(y=0; y<img_height; y+=HOG_GAP) {
        for(x=0; x<img_width; x+=HOG_GAP) {
            ...
            h[index_tmp]= h[index_tmp]+grad_mag*(1-((x+1-Xc)/cwidth))*(1-((y+1-Yc)/cwidth))*(1-((grad_or-Oc)/bin_size));
			index_tmp = y1 * hist2*nb_bins + x1*nb_bins + bin2;
            h[index_tmp]= h[index_tmp]+grad_mag*(1-((x+1-Xc)/cwidth))*(1-((y+1-Yc)/cwidth))*(((grad_or-Oc)/bin_size));
			index_tmp = y2 * hist2*nb_bins + x1*nb_bins + bin1;
            h[index_tmp]= h[index_tmp]+grad_mag*(1-((x+1-Xc)/cwidth))*(((y+1-Yc)/cwidth))*(1-((grad_or-Oc)/bin_size));
			index_tmp = y2 * hist2*nb_bins + x1*nb_bins + bin2;
            h[index_tmp]= h[index_tmp]+grad_mag*(1-((x+1-Xc)/cwidth))*(((y+1-Yc)/cwidth))*(((grad_or-Oc)/bin_size));
			index_tmp = y1 * hist2*nb_bins + x2*nb_bins + bin1;
            h[index_tmp]= h[index_tmp]+grad_mag*(((x+1-Xc)/cwidth))*(1-((y+1-Yc)/cwidth))*(1-((grad_or-Oc)/bin_size));
			index_tmp = y1 * hist2*nb_bins + x2*nb_bins + bin2;
            h[index_tmp]= h[index_tmp]+grad_mag*(((x+1-Xc)/cwidth))*(1-((y+1-Yc)/cwidth))*(((grad_or-Oc)/bin_size));
			index_tmp = y2 * hist2*nb_bins + x2*nb_bins + bin1;
            h[index_tmp]= h[index_tmp]+grad_mag*(((x+1-Xc)/cwidth))*(((y+1-Yc)/cwidth))*(1-((grad_or-Oc)/bin_size));
...
static int __getCellHogFea(const int *pGrad_x, const int *pGrad_y, int widthStep, int cellWidth, int cellHeight, float  *pCellHist, const int DirBin, int IsPi)
{
    ...
    if( (TNull == pGrad_x) || (TNull == pGrad_y) || (TNull == pCellHist))
        return -1;
...
#define DIR_BIN    9
    ...
    if ( 
		(TNull == pGradient_x)||(TNull == pGradient_y)
	  ||(TNull == pHogFea) || (TNull == pCellHist)
      ||(TNull == pCellHist)
	   )
    {
        rval = -1;
